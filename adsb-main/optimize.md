...
### 12.2 问题描述
原有的"统计分析"功能过于简单，只显示4个基础数据（在线目标、高精度信号、低质量信号、消息记录），与"态势显示"功能重复，显得鸡肋。
...
- 高度分布柱状图：按<3000m、3000-6000m、6000-9000m、9000-12000m、>12000m区间统计
- 速度分布柱状图：按<300km/h、300-500km/h、500-700km/h、700-900km/h、>900km/h区间统计

**📶 信号分析 Tab**
- 环形仪表盘：显示信号优良率百分比
- 信号质量条形图：优秀/良好/较差三档分布
- 信号详情表格：按NIC排序显示前10架飞机的航班号、NIC值、质量等级、高度、速度

**🏢 航空公司 Tab**
- 航空公司数量和总航班数统计
- 航空公司列表：显示Logo、名称、航班数量、平均NIC值
- 支持识别：南航(CZ)、国航(CA)、东航(MU)、海航(HU/BZ)、深航(ZH)、上航(FM)、川航(3U)、山航(SC)、厦航(MF)等

#### 12.3.2 新增计算属性
- `statsActiveTab`: 统计分析当前Tab状态
- `messageStats`: 消息统计追踪（总数、位置/速度消息数、每秒频率）
- `flightStats`: 飞行统计（平均/最大/最小高度和速度）
- `altitudeDistribution`: 高度分布统计
- `speedDistribution`: 速度分布统计
- `nicDistribution`: NIC信号质量分布
- `nicPieChartStyle`: 饼图动态样式（conic-gradient）
- `airlineStats`: 航空公司统计

#### 12.3.3 消息统计实时追踪
在 `handleReceivedMessage` 函数中调用 `updateMessageStats()` 实时统计位置消息和速度消息数量

#### 12.3.4 NIC饼图动态比例显示
- 修复饼图只显示红色的问题
- 使用计算属性 `nicPieChartStyle` 动态生成 conic-gradient
- 根据优秀/良好/较差的实际数量比例分配绿色/黄色/红色

### 12.4 效果
- 统计分析功能从简单4格卡片升级为专业的多维度数据分析中心
- 支持Tab切换查看不同维度的统计数据
- 实时数据动态更新，图表响应式展示
