...
| 13 | 真实 ADS-B 数据解码器 | realAdsbDecoder.ts | ✅ 完成 |
| 14 | CSV 真实数据回放功能 | Map.vue, realAdsbDecoder.ts | ✅ 完成 |
| 15 | 航班呼号识别率统计 | Map.vue | ✅ 完成 |

---

## 十三、真实 ADS-B 数据解码器

**记录时间**：2025年12月30日 10:00

### 13.1 修改文件
- 新增 `src/utils/realAdsbDecoder.ts`

### 13.2 新增内容

#### CPR 解码器 (Compact Position Reporting)
- 实现 CPR 全局解码算法（需要奇偶帧配对）
- 实现 CPR 本地解码算法（使用参考位置单帧解码）
- 实现 NL 函数计算经度区域数
- 自动缓存历史位置用于后续解码优化

#### ADS-B 消息解码器
- 支持 DF17/DF18 Extended Squitter 格式解码
- TC 1-4: 飞机识别消息解码（提取航班呼号）
- TC 9-18: 气压高度位置消息解码
- TC 19: 空中速度消息解码（地面速度/空速）
- TC 20-22: GNSS 高度位置消息解码
- NIC (Navigation Integrity Category) 计算

#### Gillham 高度编码解码
- 实现完整的 Gillham/Gray 码高度解码算法
- 支持 100 英尺分辨率的 Mode C 高度
- 正确提取 C1/A1/C2/A2/C4/A4/B1/D1/B2/D2/B4 位
- Gray 码转二进制算法实现

#### CSV 数据加载器
- 支持解析纯 hex 格式的 CSV 文件
- 自动跳过空行和注释
- 支持逗号分隔的多字段格式
...
#### 真实数据模式状态管理
- 新增 `realDataMode` 状态标识
- 新增 `realDataState` 跟踪回放状态
- 新增 `realDataProgress` 进度百分比
- 新增 `isRealDataSeeking` 拖动状态
...
#### 真实数据消息处理
- 新增 `handleRealDataMessage()` 处理解码后的数据
- 根据消息类型更新飞机位置/速度/呼号
- 自动添加轨迹点
...
#### 识别消息统计
- `messageStats` 新增 `identificationMessages` 计数器
- `updateMessageStats()` 支持 `'identification'` 类型
- 收到识别消息时自动更新统计

#### 呼号识别率计算
- 新增 `callsignStats` 计算属性
- 统计已识别飞机数量、待识别数量
- 计算识别率百分比

#### 界面显示优化
- 弹窗信息增强：显示详细表格，清楚标明呼号状态
  - ✅ 绿色表示已收到呼号
  - ⏳ 黄色表示等待识别消息
- 目标列表改进：
  - 未识别飞机显示 `[ICAO地址]` 而非 "Unknown"
  - 显示识别率统计（如 "📡 65.3% 已识别"）
  - 未识别飞机用黄色显示并带 ⏳ 标记

#### 新增样式
- `.qt-callsign-stats` 识别率标签样式
- `.qt-pending` 待识别呼号样式
- `.qt-pending-badge` 等待标记样式

### 15.3 效果
- 清楚区分"尚未收到识别消息"和"真正的未知航班"
- 用户可以看到当前识别率
- 随着回放进行，识别率会逐渐提高
